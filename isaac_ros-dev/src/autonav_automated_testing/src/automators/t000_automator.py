...
def setup_scroll_region():
    ...
    sys.stdout.write(f'\033[{STATUS_LINES + 1};r')  # scroll region starts after header
    sys.stdout.write(f'\033[{STATUS_LINES + 1};1H')  # move cursor into scroll region
    sys.stdout.flush()
...
def draw_status(odom, joy, gps, imu, collecting, test_started):
    ...
    for i, line in enumerate(lines):
        sys.stdout.write(f'\033[{i + 1};1H\033[2K{line}')  # move to row, clear, write
