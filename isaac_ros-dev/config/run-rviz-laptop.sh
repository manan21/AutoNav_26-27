...
if [[ -f /.dockerenv ]] || grep -qaE '/docker/|/containerd/' /proc/1/cgroup 2>/dev/null; then
    echo "ERROR: run-rviz-laptop.sh is for laptop-native RViz over Wi-Fi/DDS, not for the container."
    echo "Inside the Jetson container, use:"
...
echo "run-rviz-laptop.sh: launching laptop RViz over DDS/Wi-Fi"
